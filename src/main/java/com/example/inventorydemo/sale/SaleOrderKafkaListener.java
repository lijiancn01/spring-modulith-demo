...
public class SaleOrderKafkaListener {
    ...
    @KafkaListener(topics = "sale-settlement-completed", groupId = "sale-group")
    public void handleSettlementCompleted(Map<String, Object> event,
        ...
        log.info("销售模块收到结算完成事件（Kafka）: orderId={}, quantity={}, amount={}, attempts={}", orderId, quantity, amount, attempts);

        SaleOrder order = saleOrderRepository.findById(orderId)
                .orElseThrow(() -> new RuntimeException("Sale order not found: " + orderId));
